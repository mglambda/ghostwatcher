...
# timecodes don't need to be a type by themselves but we use to display time for humans and LLMS throughout
...
class Summary(BaseModel):
    """Summary of the events in a section of a video."""

    time_start: float = Field(
        default = -1.0,
        description = "The time, in seconds, at which the events that are summarized here begin in the video."
    )

    time_end: float = Field(
        default = -1.0,
        description = "The time, in seconds, at which the events summarized here end in the video."
    )
...
class Program(BaseModel, arbitrary_types_allowed=True):
    ...
    character_folder: str = Field(
        description = "The AI character to load into the ghostbox."
    )
